...
import {
  applyBackendJob,
  clearBackendSnapshot,
  getProcessingSteps,
  pollJob,
  startAnalysis,
} from "../services/analysisService.js";
import { createPipelineController } from "../services/pipelineStatus.js";
...
export function ProcessingPage() {
  ...
  useEffect(() => {
    ...
    pipelineRef.current = createPipelineController(steps, {
  ...
  const progress = useMemo(() => {
    ...
    const completed = steps.filter((step) =>
      ["Complete", "Warning", "Error"].includes(statuses[step.id])
    ).length;
  ...
  function runDemoAnalysis() {
    ...
    setRunning(true);
    pipelineRef.current?.reset(initialStatus);
    pipelineRef.current?.applyJob({ status: "pending", current_step: "queued" });
  ...
  async function handleBackendSubmit(event) {
    ...
    try {
      ...
      setJob(started);
      pipelineRef.current?.applyJob(started);

      const finished = await pollJob(started.job_id, {
        intervalMs: 2000,
        onUpdate: (updatedJob) => {
          setJob(updatedJob);
          pipelineRef.current?.applyJob(updatedJob);
        },
      });
      ...
      if (finished.status === "completed") {
        ...
        pipelineRef.current?.completeAll();
        notify("Analysis complete — loading dashboard");
        window.setTimeout(() => navigate("/dashboard"), 800);
      } else {
        pipelineRef.current?.failRemaining();
  ...
  return (
    <div className="page-stack">
      <PageHeader
        ...
        subtitle="Multi-agent ingestion and verification workflow driven by the backend analyze API."
      />

      <Card className="pipeline-shell">
        <div className="pipeline-head">
          <p className="small-label">
            MEIA Workspace — {ticker}
            {job ? ` · Job ${job.job_id?.slice(0, 8)}` : ""}
            {currentStepLabel ? ` · ${currentStepLabel}` : ""}
          </p>
          <span>{jobLoading ? `${progress}% complete` : `${progress}% complete`}</span>
        </div>
      ...
      <Card className="backend-run-card">
        <div>
          ...
          <p>
            Upload earnings assets to <code>/api/analyze</code>. Step cards follow{" "}
            <code>current_step</code> from <code>/api/jobs/&#123;job_id&#125;</code> while the job runs.
          </p>
        </div>
...
function StepCard({ active, status, step }) {
  const Icon =
    status === "Complete" ? Check : status === "Warning" || status === "Error" ? TriangleAlert : Clock;
  return (
